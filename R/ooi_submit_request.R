...
#'
#' @param url The request URL generated through the ooi_create_url function, or as a url string input by the user.
...
#' @return A JSON response object is returned if the request is successful. If it is not successful, a message with the status code is printed.
#' @examples
...
#' response = ooi_submit_request(url,user = 'OOI-API-USERNAME',token = 'OOI-API-TOKEN") 
